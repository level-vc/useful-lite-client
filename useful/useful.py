...
class Useful:
    ...
    def __init__(  # noqa: PLR0913, PLR0915
        ...
        Args:
            ...
            dev_mode (bool): Whether to upload to development environment or production.
        """
        # Initialization
        job_name = (
            job_name
        ...
        self.logger = (
            FakeCloudLogger()
            if block_uploads
            else CloudLogger(api_key or os.environ.get("USEFUL_API_KEY"), dev_mode)
        )
    ...
    def check(  # noqa: PLR0915
        ...
        def decorate(func):  # noqa: PLR0915
            ...
            def useful_wrapper(*args, **kwargs):  # noqa: PLR0912, PLR0915
                ...
                instantiations["".join(function_signature[-2:])] = self
                logging.info(f"Added to instantiations, {instantiations.keys()=}")
                ...
                for elem, val in kwargs.items():
                    ...
                    argument_dtypes[f"kwargs_{elem}"] = type(val).__name__
                for elem, val in enumerate(args):
                    ...
                    argument_dtypes[f"args_{elem}"] = type(val).__name__
                ...
                try:
                    ...
                    payload = Function(
                        ...
                        data=FunctionData(
                            ...
                            arguments_dtypes=argument_dtypes,
                            standalone_check_name=None,
                            code="".join(lines),
                ...
                try:
                    ...
                    if (check_statistics) & (type(out) != type(None)):
                        ...
                        logging.info(f"--- LOGGING STATS {function_signature} ---")
...
def standalone_check(  # noqa: PLR0915
    name_tag=None, check_statistics=True, verbose=0
):
    """
    Tracks the runtime and statistics of a function.
    ...
    def decorate(func):  # noqa: PLR0915
        ...
        def useful_wrapper(*args, **kwargs):  # noqa: PLR0912, PLR0915
            ...
            # find the last Useful decorated function in the call stack
            def process_call(full_line, search_name):
                """Find the relevant function call in the code line."""
                # Adjusted regex to match method calls with any arguments inside
                # parentheses
                pattern = re.search(
                    rf"(\b\w+(\.\w+)*\.{search_name}\([^)]*\))", full_line
                )
                if pattern:
                    return pattern.group(0)

                # If no match, then try to match without any prefix directly,
                # including arguments
                pattern = re.search(rf"(?<!\w){search_name}\([^)]*\)", full_line)
                if pattern:
                    return pattern.group(0)

                return None

            # extract information from the call stack
            stack = traceback.extract_stack()
            reprocessed_stack = []
            last_caller_info_lines = []
            search_func = func.__name__
            for caller_info in stack[::-1]:
                last_caller_info_lines.append(caller_info.line)
                usage = process_call(caller_info.line, search_func)
                if usage:
                    search_func = caller_info.name
                reprocessed_stack.append(
                    (caller_info.filename, caller_info.lineno, caller_info.name, usage)
                )

            # process information from the call stack to find the naming and
            # check name for useful
            called_within = None
            func_name_used = None
            was_last_useful = False
            logging.debug("\n\n".join([str(x) for x in reprocessed_stack]))
            is_useful_wrapper = {
                i: x[2] == "useful_wrapper" for i, x in enumerate(reprocessed_stack)
            }
            for i, caller_info in enumerate(reprocessed_stack):
                gap = "--" * i
                logging.debug(
                    f"{gap} {caller_info[0]}:{caller_info[1]}: {caller_info[2]}"
                )
                logging.debug(f"{gap} {caller_info[3]=}")
                is_next_useful = is_useful_wrapper.get(i + 1, False)
                logging.debug(f"{gap} {is_next_useful=}")
                logging.debug("")
                if (isinstance(func_name_used, str)) & is_next_useful:
                    called_within = caller_info[2]
                    break
                if (caller_info[3] is not None) & is_useful_wrapper.get(i + 2, False):
                    func_name_used = caller_info[3]

                was_last_useful = caller_info[2] == "useful_wrapper"

            if called_within is None:
                # call function, return
                logging.info(
                    f"Running standalone {func.__name__} without Useful context."
                )
                return func(*args, **kwargs)
            else:
                logging.info(f"Running standalone {func.__name__} in Useful context.")
            # otherwise, we can now rename the function and add it to the traceback
            # accordingly
            func_name_used = func_name_used.split("(")[0]
            logging.info(
                f"Called within: {called_within}, func_name_used: {func_name_used}"
            )

            safe_name_tag = func_name_used

            useful_obj = instantiations[called_within]

            indx = 1
            trial_name_tag = safe_name_tag
            while trial_name_tag in useful_obj.seen_in_runtime:
                trial_name_tag = f"{safe_name_tag} ({indx})"
                indx += 1
            useful_obj.seen_in_runtime.add(trial_name_tag)
            safe_name_tag = trial_name_tag

            hash_object = hashlib.md5(bytes(safe_name_tag, "utf-8"))  # noqa: S324

            element_name = hash_object.hexdigest()
            code = f"__{element_name}__t{str(useful_obj._Useful__get_current_micros())}"

            useful_obj.function_hash_to_name[
                "__" + hash_object.hexdigest()
            ] = safe_name_tag
            ...
            logging.info(f"[TIME] start at: {useful_obj._Useful__get_current_micros()}")
            function_signature = useful_obj.thread_signature[:]
            for item in trace:
                items = item.split("__t")
                function_signature.append(items[0])
                with contextlib.suppress(Exception):
            ...
            # error handing
            try:
                started_at = useful_obj._Useful__get_current_micros()
                tracebacks, out = locals()[code](func, *args, **kwargs)
                finished_at = useful_obj._Useful__get_current_micros()
                logging.info(f"[TIME] ended at: {finished_at}")
            except Exception as e:
                # [end] function log here
                payload = Function(
                    job=useful_obj.job,
                    name=safe_name_tag,
                    job_timestamp=useful_obj.start_timestamp,
                    started_at=started_at,
                    finished_at=useful_obj._Useful__get_current_micros(),
                    error=traceback.format_exc(),
                    data=FunctionData(
                        ...
                        arguments=json.dumps(argument_upload),
                        arguments_dtypes=argument_dtypes,
                        standalone_check_name=func.__name__,
                        line_start=code_line_start,
                        git_url=useful_obj.git_url,
                        blame=useful_obj.blame,
                        caller=useful_obj.function_hash_to_name[
                            trace[-2].split("__t")[0]
                        ]
                        if len(trace) > 1
                        else None,
                ...
                useful_obj.logger.upload_function(payload)
                useful_obj.logger.wait()
                raise e  # noqa: TRY201
            ...
            if verbose >= 1:
                ...
                for i, tr in enumerate(tracebacks):
                    if i == 0:
                        logging.info(tr.split("__t")[0] + f" ({tr.split('__t')[1]})")
                    else:
                        logging.info(
                            " " * i
                            + "|-- "
            ...
            try:
                ...
                if (check_statistics) & (type(out) != type(None)):
                    ...
                    if isinstance(stats, list):
                        for i, stat in enumerate(stats):
                            ...
                            useful_obj.logger.upload_statistic(payload)
                        number_of_return_arguments = len(stats)
                    else:
                        payload = Statistic(
                            job=useful_obj.job,
                ...
                payload = Function(
                    ...
                    data=FunctionData(
                        ...
                        arguments_dtypes=argument_dtypes,
                        code="".join(lines),
                        standalone_check_name=func.__name__,  # TODO - update
                        line_start=code_line_start,
                        git_url=useful_obj.git_url,
                        blame=useful_obj.blame,
                        caller=useful_obj.function_hash_to_name[
                ...
                logging.exception(e)  # noqa: TRY401
                if not useful_obj.ignore_execution_errors:
                    raise e  # noqa: TRY201
                pass

            useful_obj.logger.wait()  # TODO find a way to run this just once
            return out
